...
| D22 | Multi-tenant signing keys | **Signing key selection is tenant-aware, not just `adcp_use`-aware.** The L1 signing SPI must select by purpose plus caller/resolved publisher tenant context, so one operator can publish multiple tenant keys on a single JWKS endpoint: same `adcp_use`, distinct `kid` per tenant. v0.2 must freeze the API shape without assuming a singleton key per use; v0.3 wires tenant resolution from `AccountStore` / `adagents.json` into that selector. | Refines RFC §L1 signing and §L2 multi-tenant principal resolution |
...
- **Wire-version negotiation** (`adcp_version`, `supported_versions`, response echo, `VERSION_UNSUPPORTED`). Keep three values distinct: bundle pin (`3.1.0-beta.5`, full semver, build/cache key), stable wire release (`3.1`, major.minor), and prerelease wire token (`3.1-beta.5`, major.minor-prerelease while the beta line is live). Current Java code only models the stable `major.minor` form, so v0.1 must expand the version model before 3.1 beta traffic is supported. Java must emit both `adcp_version` and legacy `adcp_major_version` through 3.x, pick response validators by served version, and route unversioned legacy traffic to `3.0`. → [`transport`](#track-3--l0-transport-mcp--a2a) + [`codegen`](#track-2--l0-types--codegen) + [`testing`](#track-9--testing--conformance).
...
- **Open error-code decoding.** Receivers must not fail closed on unknown `error.code`; classify from `recovery` and default conservatively when absent. Java generated enums need an unknown-value strategy, not a hard enum parse failure. → [`codegen`](#track-2--l0-types--codegen) + [`transport`](#track-3--l0-transport-mcp--a2a).
...
**Milestone targets:** v0.1 needs full generated type coverage for the L0 surface, `3.1.0-beta.5` target-bundle coverage (or 3.1 GA if it cuts first), 3.0 + 3.1 validator selection, and stable/prerelease wire version normalization wired into transport.
...
- Wire `adcp_version` on every request/response, accepting and echoing both stable values (`3.1`) and prerelease values (`3.1-beta.5`) while storing the full bundle pin (`3.1.0-beta.5`) separately from the wire token. Keep the legacy `adcp_major_version` mirror through 3.x. Server dispatch validates requested version before handler execution and returns typed `VERSION_UNSUPPORTED` with `supported_versions`.
- Central response-envelope stamping for sync success, submitted, failed, and advisory-success responses. Payload `errors[]` advisories like `STALE_RESPONSE` must not be thrown as transport failures.
...
- 3.1 action-discovery helper layer: parse `allowed_actions[]` and `available_actions[]`, decompose `update_media_buy` requests into concrete mutations, and surface `ACTION_NOT_ALLOWED` with structured details before handler side effects.
...
- Version-aware compliance cache selection: a 3.1 runner can execute supplied 3.0 bundles with matching schema cache instead of assuming the installed SDK's primary pin.
...
**Milestone targets:** v0.1 ships the storyboard runner, mock-server forwarding adapter, 3.1 output splits/notices, and version-aware compliance cache selection. Conformance test surface expands at each subsequent milestone as L1/L2/L3 land.
...
11. **Design partner outreach.** Anchor candidates by audience segment: one publisher running Spring Boot, one SSP, one broadcaster middleware team. ADvendio is a concrete reviewer candidate for the Spring Boot 3.x / OAuth 2.1 / multi-tenant signing shape, but has not committed engineering time or an LOI; still need 2–3 committed design partners before scaling.
...
| Confirmed decisions D1–D23 locked | ✅ |
